...
dfzip = df_yr_built.loc[
        df_yr_built['zipcode'] == zipcode_slider, ['zipcode', 'price_m2', 'price_inc_perc_year']].groupby('price_inc_perc_year').mean().reset_index()
st.write(dfzip)
...
st.write(f'O cresimento mês a mês dos imóveis com 3 banheiros é de {df_inc_month:.2f}% ao mês nos anos de 2014 e 2015.')
...
df_inc_bathrooms = df_yr_built.loc[
    df_yr_built['bathrooms'] == 3, ['id', 'bathrooms', 'price_m2', 'price_inc_perc_month', 'month_year','Year']].groupby('Year').median()
df_no_inc_bathrooms = df_yr_built.loc[
    df_yr_built['bathrooms'] == 3, ['id', 'bathrooms', 'price_m2', 'price_inc_perc_month', 'month_year','Year']].groupby('Year').median().sort_values('Year', ascending=False)
...
df_zipcode = df_yr_built.loc[
    (df_yr_built['zipcode'] == zip_year_slider), [ 'price_m2', 'price', 'zipcode', 'price_inc_perc_year', 'Year']].groupby(
...
#
st.write('o zipcode com maior alta é o 98039 e os imóveis com maior potencial de ganho foram baseados na vista e nas condições do imóvel.')
...
df_zipco = df_yr_built.loc[
    (df_yr_built['zipcode'] > 98000), ['id', 'price_m2', 'price', 'zipcode', 'price_inc_perc_year', 'Year']].groupby(
...
(df_yr_built['price_m2'] < 367) & (df_yr_built['zipcode'] == 98168) & (df_yr_built['price_inc_perc_year'] < 0) & (
            df_yr_built['price_inc_perc_year'] < -90), :'yr_built'].sort_values('price_inc_perc_year', ascending=True)
